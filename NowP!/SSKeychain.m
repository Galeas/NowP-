...
#pragma mark - Deleting Entries

+ (BOOL)deleteEntryForService:(NSString *)serviceName account:(NSString *)account
{
    return [self deleteEntryForService:serviceName account:account error:nil];
}

+ (BOOL)deleteEntryForService:(NSString *)serviceName account:(NSString *)account error:(NSError *__autoreleasing *)error
{
    OSStatus status = SSKeychainErrorBadArguments;
    if (serviceName && account) {
        ...
        status = SecKeychainFindGenericPassword(NULL,
                                                (UInt32)strlen(serviceCString),
                                                serviceCString,
                                                (UInt32)strlen(accountCString),
                                                accountCString,
                                                &passwordLength,
                                                &password,
                                                &item);
